...
4. Basic Playwright/Cypress smoke coverage for the two new live-write flows (account creation, loan assessment submission) — these are exactly the kind of feature where a unit test can't catch a live schema-cache or RLS-policy misconfiguration (BUG-014 was invisible to u     nit tests).
